...
import { ClassAnnotationNode, EntityNode, FairLeadNodeOptions, SchemaNode, StaticClassNode } from '@/fairlead/logic-presets/classic/nodes';
...
type Node = StaticClassNode;
type Conn =
  | Connection<StaticClassNode, StaticClassNode>;
...
//  area.use(minimap);
  ...
  let nodeLookup = new Map<string, StaticClassNode>();

  for (let class_ of classes) {
    let node = new StaticClassNode(class_, schemaName, fairleadOptions);
  ...
  for (let class_ of classes) {
    for (let [key, relation] of Object.entries(class_.relations) as [string, SlotDefinitionView][]) {
      ...
      let a = nodeLookup.get(subject_name) as StaticClassNode;
      let b = nodeLookup.get(object_name) as StaticClassNode;
