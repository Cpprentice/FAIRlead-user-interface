...
import { ClassAnnotationNode, EntityNode, FairLeadNodeOptions, SchemaNode, StaticEntityNode } from '@/fairlead/logic-presets/classic/nodes';
// import { EntityProvider, SchemaProvider, ClassProvider } from '@/providers/schema_api';
...
type Node = EntityNode | SchemaNode | ClassAnnotationNode;
type Conn =
  | Connection<EntityNode, EntityNode>
  | Connection<SchemaNode, EntityNode>
  | Connection<ClassAnnotationNode, ClassAnnotationNode>
...
export async function createEditor(container: HTMLElement, schemaName: string, classes: ClassDefinitionView[]) {
...
//  area.use(minimap);
  ...
  let nodeLookup = new Map<string, ClassAnnotationNode>();

  for (let class_ of classes) {
    let node = new ClassAnnotationNode(class_, schemaName, fairleadOptions);
    nodeLookup.set(class_.name, node);
    await editor.addNode(node);
  }

  for (let class_ of classes) {
    for (let [key, relation] of Object.entries(class_.relations) as [string, SlotDefinitionView][]) {
      let subject_name = class_.name;  // getEntityIdFromUrl(relation.hasSubjectEntity)
      let object_name = relation.range;  // getEntityIdFromUrl(relation.hasObjectEntity)
      let a = nodeLookup.get(subject_name) as ClassAnnotationNode;
      let b = nodeLookup.get(object_name) as ClassAnnotationNode;
      let outputName = `${relation.name}-${object_name}`;
      if (b == undefined) {
